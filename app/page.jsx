...
export default function Page(){
  ...
  const shown=stocks.filter(s=>{
    if(sectorFilter && s.s!==sectorFilter) return false;
    if(filter==="up" && !(s.c>0)) return false;
    if(filter==="down" && !(s.c<0)) return false;
    if(search){ const q=search.toLowerCase(); if(!s.t.toLowerCase().includes(q) && !s.n.toLowerCase().includes(q)) return false; }
    return true;
  });
  function openSector(name){ setSectorFilter(name); setFilter("alle"); setSearch(""); setTimeout(()=>{ const el=document.getElementById("markeder"); if(el) el.scrollIntoView({behavior:"smooth"}); },50); }
    ...
    <div className="root">
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .secbar.clickable{cursor:pointer;transition:border-color .2s,transform .2s;}
        .secbar.clickable:hover{border-color:rgba(245,166,35,.5);transform:translateY(-2px);}
        .secbar .arr{color:var(--gold);font-size:12px;opacity:.7;}
        .secpill{font-size:12px;color:var(--gold);background:rgba(245,166,35,.12);border:1px solid rgba(245,166,35,.3);padding:3px 10px;border-radius:20px;margin-left:8px;cursor:pointer;font-family:'Inter';font-weight:500;letter-spacing:0;}
        ...
        .clearbtn{background:none;border:1px solid var(--line);color:var(--mut);padding:10px 16px;border-radius:11px;font-size:13px;cursor:pointer;font-family:inherit;}
        .clearbtn:hover{color:var(--gold);border-color:var(--gold);}
        ...
        <div className="secgrid">
          {sectors.map((x,i)=>(
            <Reveal key={x.s} delay={i*55}>
              <div className="secbar clickable" onClick={()=>openSector(x.s)}>
                <div className="row"><span className="nm">{x.s} <span className="arr">→</span></span><span className={`mono ${x.avg>=0?"up":"down"}`}>{x.avg>=0?"+":""}{x.avg.toFixed(2)}%</span></div>
      ...
      <section className="sec" id="markeder">
        <Reveal>
          <div className="sec-h">
            <div>
              <div className="sec-t">Markeder {sectorFilter && <span className="secpill" onClick={()=>setSectorFilter(null)}>{sectorFilter} ✕</span>}</div>
              <div className="sec-d">{sectorFilter?`Viser sektoren ${sectorFilter}`:"Tryk på en aktie for kurser, graf og nyheder"}</div>
            </div>
            <div className="seg">
        ...
        <div className="toolbar">
          <input className="searchbox" placeholder="Søg aktie (navn eller ticker)…" value={search} onChange={(e)=>setSearch(e.target.value)} />
          {(sectorFilter||search) && <button className="clearbtn" onClick={()=>{ setSectorFilter(null); setSearch(""); }}>Vis alle</button>}
        </div>
        <div className="cards">
        ...
        {shown.length===0 && <div className="empty">Ingen aktier matcher. Tryk “Vis alle”.</div>}
      </section>
